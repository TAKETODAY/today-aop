...
public class CodeEmitter extends LocalVariablesSorter {
	...
	public void super_invoke(Signature sig) {
		emit_invoke(Constant.INVOKESPECIAL, ce.getSuperType(), sig);
	}
	...
	public void invoke_interface(Type owner, Signature sig) {
		emit_invoke(Constant.INVOKEINTERFACE, owner, sig);
	}

	public void invoke_virtual(Type owner, Signature sig) {
		emit_invoke(Constant.INVOKEVIRTUAL, owner, sig);
	}

	public void invoke_static(Type owner, Signature sig) {
		emit_invoke(Constant.INVOKESTATIC, owner, sig);
	}
	...
	public void invoke_constructor(Type type, Signature sig) {
		emit_invoke(Constant.INVOKESPECIAL, type, sig);
	}
		...
		if (TypeUtils.isPrimitive(type)) {
			if (type == Type.VOID_TYPE) {
				...
				invoke_constructor(boxed,
						new Signature(Constant.CONSTRUCTOR_NAME, Type.VOID_TYPE, new Type[] { type }));
			}
		}
	}
