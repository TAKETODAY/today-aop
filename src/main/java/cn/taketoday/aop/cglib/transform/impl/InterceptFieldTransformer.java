...
public class InterceptFieldTransformer extends ClassEmitterTransformer {
	...
	private static final Type ENABLED = TypeUtils
			.parseType("cn.taketoday.aop.cglib.transform.impl.InterceptFieldEnabled");
	...
	private static Signature readCallbackSig(Type type) {
		...
		return new Signature("read" + callbackName(remap), remap,
				new Type[] { Constant.TYPE_OBJECT, Constant.TYPE_STRING, remap });
	}

	private static Signature writeCallbackSig(Type type) {
		Type remap = remap(type);
		return new Signature("write" + callbackName(remap), remap,
				new Type[] { Constant.TYPE_OBJECT, Constant.TYPE_STRING, remap, remap });
	}
